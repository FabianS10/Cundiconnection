...
function hasAllowedDomain(email) { return normalizeEmail(email).endsWith(`@${ALLOWED_DOMAIN}`); }
function str(value, fallback = '') { return typeof value === 'string' ? value.trim() : fallback; }

async function seedAdmin() {
  const email = normalizeEmail(process.env.FIRST_ADMIN_EMAIL || 'fandressabogal@ucundinamarca.edu.co');
  ...
  await run(
    ...
    [email, passwordHash, 'Fabian Sabogal', 'Fusagasugá', 'Ingeniería de Sistemas',
     'Founder Mode', 'Admin gatekeeper de Cundiconnection.', 'AI, startups, dark UI', 'violet']
  );
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, app: 'Cundiconnection API', allowedDomain: ALLOWED_DOMAIN });
});
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!hasAllowedDomain(email))
      return res.status(400).json({ message: `Only @${ALLOWED_DOMAIN} emails can request access.` });
    if (password.length < 8)
      return res.status(400).json({ message: 'Password must be at least 8 characters.' });

    const existing = await get('SELECT id FROM users WHERE email = ?', [email]);
    if (existing) return res.status(409).json({ message: 'This email already requested access or exists.' });
    ...
    await run(
      ...
      [email, passwordHash, displayName, str(req.body.campus, 'Fusagasugá'),
       str(req.body.program), str(req.body.semester), str(req.body.bio), str(req.body.interests), gradient]
    );
    res.status(201).json({ message: 'Access request received. The admin must verify and approve your profile before login.' });
  } catch (error) {
    console.error(error);
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (user.status === 'pending') return res.status(403).json({ message: 'Your profile is pending admin approval.' });
    if (user.status === 'rejected') return res.status(403).json({ message: 'Your access request was rejected.' });
...
app.patch('/api/me', requireAuth, async (req, res) => {
  try {
    await run(
      ...
      [str(req.body.displayName, req.user.display_name), str(req.body.campus, req.user.campus),
       str(req.body.program, req.user.program), str(req.body.semester, req.user.semester),
       str(req.body.bio, req.user.bio), str(req.body.interests, req.user.interests), req.user.id]
    );
...
app.post('/api/swipes', requireAuth, async (req, res) => {
  try {
    ...
    const mutual = action === 'like'
      ? await get(`SELECT id FROM swipes WHERE swiper_id = ? AND swiped_id = ? AND action = 'like'`,
...
// ── Rich stats for analytics panel ───────────────────────────────────────────
app.get('/api/admin/stats', requireAuth, requireAdmin, async (_req, res) => {
  try {
    const [totals, daily, campuses, programs, swipeStats, matchCount] = await Promise.all([
      ...
      `),
      // Top campuses
      all(`SELECT campus, COUNT(*) as count FROM users WHERE status='approved' AND campus != '' GROUP BY campus ORDER BY count DESC LIMIT 8`),
      // Top programs
      all(`SELECT program, COUNT(*) as count FROM users WHERE status='approved' AND program != '' GROUP BY program ORDER BY count DESC LIMIT 8`),
      // Swipe breakdown
      all(`SELECT action, COUNT(*) as count FROM swipes GROUP BY action`),
      // Total matches
      get(`
        SELECT COUNT(*) as count FROM (
          SELECT DISTINCT CASE WHEN s1.swiper_id < s2.swiper_id THEN s1.swiper_id||'-'||s2.swiper_id ELSE s2.swiper_id||'-'||s1.swiper_id END as pair
          FROM swipes s1
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      filledDaily.push({ day: key, count: found ? found.count : 0 });
    }
...
app.post('/api/admin/users/:id/decision', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if (target.role === 'admin' && decision === 'rejected')
      return res.status(400).json({ message: 'Admin accounts cannot be rejected from here.' });
...
await seedAdmin();
app.listen(PORT, () => {
  console.log(`🔥 Cundiconnection API on http://localhost:${PORT}`);
});
